import { Offer, Explainability } from "./types";
...
export function termsUtility(config: NegotiationConfig, terms: "Net 30" | "Net 60" | "Net 90") {
...
export function totalUtility(config: NegotiationConfig, offer: Offer) {
  ...
  const pu = offer.unit_price == null ? 0 : priceUtility(config, offer.unit_price);
  const tu = offer.payment_terms == null ? 0 : termsUtility(config, offer.payment_terms);
...
export function computeExplainability(
  ...
  const total = (weightedPrice == null || weightedTerms == null) 
    ? null 
